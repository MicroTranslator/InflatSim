    ...
    int64_t cmp_jmp_fusion_opt_sum_ex(TBCount *tbcnt) {
        ...
        for (int insni=1; insni<n_insns; insni++) {
            ...
            if (head->id==X86_INS_CMP && isSignedOrUnsignedJcc(tail->id))
                opt -= 1 * count;
            else if ((head->id==X86_INS_ADD || head->id==X86_INS_SUB || head->id==X86_INS_INC || head->id==X86_INS_DEC) && isSignedOrUnsignedJcc(tail->id))
                opt -= 1 * count;
...
protected:
    ...
    int inflt_extra_load_plus_x(PCCount *pccnt, int x) {
        int mem = 0;
    ...
    int inflt_extra_load_plus_1(PCCount *pccnt) {
        return inflt_extra_load_plus_x(pccnt, 1);
    }
    ...
    int inflt_extra_ret(PCCount *pccnt) {
        ...
        for (int i=0; i<x86->op_count; i++) {
            ...
            if (op->type == X86_OP_IMM) {
                immload = 2; // if ret_i then extra inflate
                break;
            }
        }
...
public:
    ...
    BaseExtra inflt(PCCount *pccnt) override {
        ...
        case X86_INS_AND: case X86_INS_OR: case X86_INS_XOR:
            return BaseExtra(1, inflt_extra_default(pccnt));
        case X86_INS_CALL:
            return BaseExtra(3, inflt_extra_load_plus_1(pccnt));
        case X86_INS_RET:
            return BaseExtra(2, inflt_extra_ret(pccnt));
        ...
        case X86_INS_JMP:
            return BaseExtra(1, inflt_extra_load_plus_x(pccnt, 2));
        case X86_INS_LAHF:
            return BaseExtra(1, 0);
        // TODO: r16,[m] inflt=2
        case X86_INS_LEA:
        ...
        case X86_INS_MOVSX: case X86_INS_MOVZX: case X86_INS_MOVSXD:
            return BaseExtra(1, inflt_extra_store_plus_1(pccnt));
        // TODO: ...
        case X86_INS_MUL:
        ...
        case X86_INS_PUSH:
            return BaseExtra(2, inflt_extra_push(pccnt));
        ...
        case X86_INS_SUBSS: case X86_INS_SUBSD:
            return BaseExtra(1, inflt_extra_load_plus_1(pccnt));
        ...
        case X86_INS_CVTSI2SD :
            return BaseExtra(2, 0);
        // TODO: x,r64 needs extra inflation
